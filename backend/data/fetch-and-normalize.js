...
// ------------------------------------------------------------
// MAIN PIPELINE (TWO PHASES)
// ------------------------------------------------------------
async function run() {
  const apiKey = process.env.SPOONACULAR_KEY;

  // -----------------------------------------
  // PHASE 1 → FETCH RAW RECIPES
  // -----------------------------------------
  ...
  // Write raw recipes to fetchedRecipes.json
  const fetchedPath = path.join(process.cwd(), "db", "fetchedRecipes.json");
  const dbDir = path.join(process.cwd(), "db");
  if (!fs.existsSync(dbDir)) fs.mkdirSync(dbDir);

  fs.writeFileSync(fetchedPath, JSON.stringify(rawRecipes, null, 2), "utf8");
  console.log(`📥 Raw recipes written to ${fetchedPath}`);

  // -----------------------------------------
  // PHASE 2 → NORMALIZE RECIPES
  // -----------------------------------------
  console.log("🔧 Normalizing recipes...");
  const normalized = rawRecipes.map(r => normalizeRecipe(r));

  const cleanedPath = path.join(process.cwd(), "db", "recipes-cleaned.json");
  fs.writeFileSync(cleanedPath, JSON.stringify(normalized, null, 2), "utf8");

  console.log(`🎉 Normalized recipes written to ${cleanedPath}`);
}
